...
from p2pfl.learning.tensorflowlite.keras_tflite_dataset import KerasExportStrategy
from p2pfl.learning.tensorflowlite.keras_tflite_logger import FederatedLogger
from p2pfl.learning.tensorflowlite.keras_tflite_model import KerasTfliteModel
from p2pfl.management.logger import logger


class KerasTfliteLearner(NodeLearner):
    """
    Learner for TensorFlow/Keras models in P2PFL.

    Args:
        model: The KerasTfliteModel instance.
        data: The P2PFLDataset instance.
        self_addr: The address of this node.

    """

    def __init__(self, model: KerasTfliteModel, data: P2PFLDataset, self_addr: str = "unknown-node") -> None:
    ...
    def set_model(self, model: Union[P2PFLModel, List[np.ndarray], bytes]) -> None:
        ...
        """
        if isinstance(model, KerasTfliteModel):
            self.model = model
        elif isinstance(model, (list, bytes)):
            self.model.set_parameters(model)

    def get_model(self) -> KerasTfliteModel:
        """
